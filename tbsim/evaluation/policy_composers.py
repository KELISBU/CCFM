...
class ReplayAction(PolicyComposer):
    ...
    def get_policy(self):
        ...
        elif self.eval_config.env == "nuplan":
            exp_cfg = get_registered_experiment_config("nuplan_diffusion")
        else:
...
class Flowmatching(PolicyComposer):
    """Diffusion model for Controllable Traffic Simulation(CTG)"""
    
    def get_policy(self, policy=None, modify_config = None):
        if policy is not None:
            assert isinstance(policy, FlowmatchingTrafficModel)
            policy_cfg = None
        else:
            policy_ckpt_path, policy_config_path = get_checkpoint(
                ckpt_key=self.eval_config.ckpt.planner.ckpt_key,
            ...
            policy_cfg["algo"]["do_guidance"] = self.eval_config.guidance
            policy_cfg["algo"]["guide_config"]["guidance_fn"]   = self.eval_config.guidance_fn
            policy = FlowmatchingTrafficModel.load_from_checkpoint(
                policy_ckpt_path,
                algo_config=policy_cfg.algo,
                modality_shapes=self.get_modality_shapes(policy_cfg),
            ).to(self.device).eval()
...
class FM(Flowmatching):
    """Flow matching + CCFM constraint projection for safety-critical generation."""

    def get_policy(self, policy=None, modify_config=None):
        policy, policy_cfg = super().get_policy(policy, modify_config)
        # Enable guidance so RasterizedFMModel.sample() activates CCFM projection
        policy.nets["policy"].do_guidance = True
        return policy, policy_cfg
...
class StrivePolicy_trajdata(PolicyComposer):
    def get_policy(self, policy=None):
        env = getattr(self.eval_config, "env", "nusc")
        dataset = "nuplan" if env == "nuplan" else "nuscenes"
        policy = StrivePlanner_trajdata(self.device, dataset=dataset)
        return policy, None
...
class STRIVE(PolicyComposer):
    def get_policy(self, policy=None, modify_config=None):
        ...
        # Set up guidance if provided via modify_config
        if modify_config is not None and hasattr(modify_config, 'guide_config') and modify_config.guide_config:
            from tbsim.utils.guidance_utils import initialize_loss_calculator
            guidance_fn = modify_config.guide_config.get('guidance_fn', [])
            if guidance_fn:
                guidance_config_list = [{'name': fn} for fn in guidance_fn]
                policy.set_guidance(guidance_config_list)
